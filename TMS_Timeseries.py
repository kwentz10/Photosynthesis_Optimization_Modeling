...
# -*- coding: utf-8 -*-
"""
Created on Wed May 31 13:18:38 2017
...
from time_dep_params import surtemp_dm_dy_smooth, surtemp_mm_dy_smooth, surtemp_wm_dy_smooth, vwc_dm_dy_smooth, vwc_mm_dy_smooth, vwc_wm_dy_smooth, na_dm_dy_smooth, na_mm_dy_smooth, na_wm_dy_smooth,       surtemp_dm_smooth, surtemp_mm_smooth, surtemp_wm_smooth, vwc_dm_smooth, vwc_mm_smooth, vwc_wm_smooth, na_dm_smooth, na_mm_smooth, na_wm_smooth 
...
#---------------Run through time series---------------#
 #dry meadow
            ...
            for xx in range(len(params)):
                for yy in range(len(params[xx])):
                    ...
                    if xx==0: 
                        ...
                        pa_v=611*np.exp((17.27*surtemp_dm[xxx][time])/(surtemp_dm[xxx][time]+237.3)) #saturation vapor pressure of air (Pa)
                        ...
                        #------calculate vapor pressure-----#
                        pa_v=611*np.exp((17.27*surtemp_mm[xxx][time])/(surtemp_mm[xxx][time]+237.3)) #saturation vapor pressure of air (Pa)
                        ea_str=pa_con_atmfrac(pa_v,3528) #saturation vapor pressure of air (Pa-->umol h20/mol air)
                        ea=rh*ea_str #vapor pressure (umol h2O/mol air)                
                        ...
                        vpd_m+=[dd[0]]
                        if xxx==0:
                            A_tot_mm+=(A[0]*3600*6)/1000000.*44.    
                        if xxx==1:
                        ...
                        #------calculate vapor pressure-----#
                        pa_v=611*np.exp((17.27*surtemp_wm[xxx][time])/(surtemp_wm[xxx][time]+237.3)) #saturation vapor pressure of air (Pa)
                        ea_str=pa_con_atmfrac(pa_v,3528) #saturation vapor pressure of air (Pa-->umol h20/mol air)
                        ea=rh*ea_str #vapor pressure (umol h2O/mol air)                
    ...
    #---------------Plot Plant Communities vs. NUE & WUE---------------#      
        ...
        tot_nue_dm_max2+=[tot_nue_dm_max]
        tot_wue_dm_max2+=[tot_wue_dm_max]
        tot_A_dm_max2+=[tot_A_dm_max]
    
        
        #moist meadow
        tot_nue_mm_avg2+=[tot_nue_mm_avg]
        tot_wue_mm_avg2+=[tot_wue_mm_avg]
        tot_A_mm_avg2+=[tot_A_mm_avg]
        tot_nue_mm_min2+=[tot_nue_mm_min]
        tot_wue_mm_min2+=[tot_wue_mm_min]
        tot_A_mm_min2+=[tot_A_mm_min]
        tot_nue_mm_max2+=[tot_nue_mm_max]
        tot_wue_mm_max2+=[tot_wue_mm_max]   
        tot_A_mm_max2+=[tot_A_mm_max]
      
    
        #wet meadow
        tot_nue_wm_avg2+=[tot_nue_wm_avg]
        tot_wue_wm_avg2+=[tot_wue_wm_avg]
        tot_A_wm_avg2+=[tot_A_wm_avg]
        tot_nue_wm_min2+=[tot_nue_wm_min]
        tot_wue_wm_min2+=[tot_wue_wm_min]
        tot_A_wm_min2+=[tot_A_wm_min]
        tot_nue_wm_max2+=[tot_nue_wm_max]
        tot_wue_wm_max2+=[tot_wue_wm_max]
        tot_A_wm_max2+=[tot_A_wm_max] 
...
#    axK.legend()
#    
#    axL.plot(days,na_dm_dy,'k-',linewidth=5,label="dry meadow")
#    axL.plot(days,na_mm_dy,'g-',linewidth=5,label="moist meadow")
#    axL.plot(days,na_wm_dy,'b-',linewidth=5,label="wet meadow")
#    axL.legend()
